...
func (m *savingsMeter) load() {
	...
	if err != nil {
		if errors.Is(err, os.ErrNotExist) {
			log.Printf("allrouter savings: state file %s missing; starting fresh", m.file)
		} else {
			log.Printf("allrouter savings: read state file %s failed: %v; starting fresh", m.file, err)
		}
		m.state = fresh
		m.currentSavedMicro.Store(0)
		return
	...
	if err := json.Unmarshal(data, &loaded); err != nil {
		log.Printf("allrouter savings: state file %s is corrupt: %v; starting fresh", m.file, err)
		m.state = fresh
		m.currentSavedMicro.Store(0)
		return
	}
	...
	for {
		...
		case <-ticker.C:
			if err := m.FlushIfDirty(); err != nil {
				log.Printf("allrouter savings: write state file %s failed: %v", m.file, err)
			}
		case <-m.stop:
			if err := m.FlushIfDirty(); err != nil {
				log.Printf("allrouter savings: write state file %s failed: %v", m.file, err)
