...
// that is safe to merge: a single choice whose delta is string content plus at
// most a constant "role", with no finish_reason, logprobs, or usage. Everything
// else flushes any pending content chunk and is written through byte-for-byte,
// so malformed frames and non-chat SSE fail open rather than being rewritten or
// dropped.
//
// The "role" tolerance is a deliberate divergence from the TrustedRouter
// enclave (which targets OpenAI-conformant cloud providers): ollama repeats
// role:"assistant" in every delta rather than only the first, so a content-only
// requirement would never merge an ollama stream — the primary local server.
...
//
// It cuts egress bytes over a WAN link (e.g. when AllRouter is exposed via
...
// time-to-first-token is unchanged. Ported from the TrustedRouter enclave.
...
// newSSEBatchWriter returns a coalescing writer, or a transparent pass-through
// when interval <= 0 (coalescing disabled). maxBytes caps the buffered content
// before a forced flush; a non-positive value falls back to 4096.
func newSSEBatchWriter(w io.Writer, interval time.Duration, maxBytes int) io.WriteCloser {
	if interval <= 0 {
...
func batchChoice(obj map[string]any) (map[string]any, map[string]any, bool) {
	...
	// Allow an optional constant "role" alongside content (ollama sends it in
	// every delta). Reject any other field — reasoning/reasoning_content,
	// tool_calls, function_call, refusal — so those deltas relay byte-for-byte
	// and are never merged into a content chunk or dropped.
	for key := range delta {
