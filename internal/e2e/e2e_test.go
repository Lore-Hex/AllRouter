...
func TestBinaryBackupRouterClaudeCodeRequestAddsOrderedFallbacks(t *testing.T) {
	t.Parallel()
	...
	raw := `{"model":"anthropic/claude-sonnet-5","max_tokens":16,"messages":[{"role":"user","content":"PONG"}]}`
	resp, body := postMessages(t, proc, raw, nil)
	if resp.StatusCode != http.StatusOK {
		t.Fatalf("status = %d body=%s", resp.StatusCode, body)
	}
	...
	if err := json.Unmarshal(trLog.last(t).Body, &forwarded); err != nil {
		t.Fatalf("decode forwarded request: %v", err)
	}
	if forwarded.Model != "anthropic/claude-sonnet-5" {
		t.Fatalf("primary model = %q", forwarded.Model)
	}
	if got, want := strings.Join(forwarded.Models, ","), "moonshotai/kimi-k3,z-ai/glm-5.2"; got != want {
		t.Fatalf("fallback models = %q, want %q", got, want)
	}
}
