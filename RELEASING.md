# Releasing HybridRouter

HybridRouter releases are tag-driven.
...
3. GitHub Actions runs GoReleaser for `v*` tags. GoReleaser builds darwin, linux, and windows archives, writes checksums, creates the GitHub release from the tag, and publishes the Homebrew formula to `Lore-Hex/homebrew-tap` as `hybridrouter`.
...
```bash
docker build --build-arg VERSION="$(git describe --tags --always --dirty)" -t hybridrouter:local .
```
...
docker run --rm -p 8383:8383 \
  -e HYBRID_LOCAL_URL=http://host.docker.internal:11434 \
  hybridrouter:local
```
